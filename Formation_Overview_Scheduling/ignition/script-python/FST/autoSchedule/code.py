...
def autoSchedStep1():
	...
	useManOrder = False
	FST.autoSchedule.mySlowFunction(useManOrder)
	...
	FST.autoSchedule.updateStatusBar("Completed Step 1", 0, 0)
	return True
...
def autoSchedStep2():
	...
	FST.autoSchedule.updateStatusBar("Sorting by line...", 1, 1)
	FST.autoSchedule.sort_SeparateLines() #separate lines
	FST.autoSchedule.updateStatusBar("Completed Step 2", 0, 0)
	return True

def autoBackgroundOptimize():
	FST.autoSchedule.updateStatusBar("Clearing schedules...", 1, 1)
	FST.autoSchedule.clearSchedules()
	dsTag = system.tag.readBlocking(["[default]Formation/Tables/Schedule/currentScheduleFromExcel"])
	...
	useManOrder = True
	FST.autoSchedule.mySlowFunction(useManOrder)
	#FST.autoSchedule.updateStatusBar("Evaluating options...", 1)
	FST.autoSchedule.setTableData(useManOrder)
	#FST.autoSchedule.updateStatusBar("Sorting by line...", 1)
	FST.autoSchedule.sort_SeparateLines() #separate lines
	FST.autoSchedule.sendToSchedule()
	countTag = system.tag.readBlocking(["[default]Formation/Tables/Schedule/Background/countUpdate"])
	count = countTag[0].value
	count = count + 1
	system.tag.writeBlocking(["[default]Formation/Tables/Schedule/Background/countUpdate"],[count])
	FST.autoSchedule.updateStatusBar("Completed Optimization", 0, 0)
	
def clearTables():
	tag = system.tag.readBlocking(["[default]Formation/Tables/Schedule/currentScheduleFromExcel"])
	data = tag[0].value
	table = 0
	rowCount = data.getRowCount()
	for row in range(rowCount):
		data = system.dataset.setValue(data, row,colTable, table)
	system.tag.writeBlocking(["[default]Formation/Tables/Schedule/currentScheduleFromExcel"],[data])

def sendToSchedule():
	FST.autoSchedule.updateStatusBar("Sending to schedule..", 1, 1)
	FST.autoSchedule.initializeTest()
	tag = system.tag.read("[default]Formation/Tables/Schedule/currentScheduleFromExcel")
	data = tag.value
	dataset = data
	...
	system.tag.write("[default]Formation/Tables/Schedule/currentScheduleFromExcel",dataWrite)
...
def clearSchedules():
	...
	if data.getRowCount() > 0:
		for row in range(data.getRowCount()):
			...
			newRows.append( [ priority, notes,sku,group,formHrs,refillAcid,bathLevel,qty,qty,batch,eastWest,table,startTime,setAuto,startOrder ] ) 
	...
	dataPrep = system.tag.read("[default]Formation/Tables/Schedule/currentScheduleFromExcel")
	dataNew = system.dataset.addRows(dataPrep.value,newRows)
	system.tag.write("[default]Formation/Tables/Schedule/currentSchedulefromExcel",dataNew)
...
def initializeSched(dataset):
	#column header names
	try:
		colHrIdx = dataset.getColumnIndex(" HRS ")
		nameFormHrs = " HRS "
	except:
		nameFormHrs = "HRS"
	try:
...
def setTableData(useManOrder):
	...
	tag = system.tag.readBlocking(["[default]Formation/Tables/Schedule/currentScheduleFromExcel"])
	data = tag[0].value
	system.tag.writeBlocking(["[default]Formation/Tables/Schedule/currentScheduleFromExcelTemp"],[data]) #copy main to temp

	singles = 0
	rowCount = data.getRowCount()
	for row in range(rowCount): #test each row for single - group focused
		tag = system.tag.readBlocking(["[default]Formation/Tables/Schedule/currentScheduleFromExcelTemp"])
		data = tag[0].value
				...
				for j in range(row+1,rowCount):
					...
					if setAuto == 1 and data.getValueAt(row,colGroup) == data.getValueAt(j,colGroup): #if next row matches, and that one is not single
						...
						if j+1 < rowCount and j-1 >= 0 and data.getValueAt(j,colGroup) != data.getValueAt(j+1,colGroup) and data.getValueAt(j,colGroup) != data.getValueAt(j-1,colGroup): #found single
							...
							system.tag.writeBlocking(["[default]Formation/Tables/Schedule/currentScheduleFromExcelTemp"],[data])
							FST.autoSchedule.mySlowFunction(useManOrder) #rerun autosched main script for comparison
...
def sort_smartShuffle():
	...
	dataPy = sorted(dataPy,reverse=False, key=operator.itemgetter(colLine,colHot))#sort by priority
	...
	start2 = 1.0 #formation side 2
	for x in slice1:
		if x[colTable] <= lineMax1:
			isLine1 = True
		else:
			isLine1 = False
		if isLine1:
	...
	shuffle = prio1 + others + setAutoOff
	shuffle = sorted(shuffle,reverse=True, key=operator.itemgetter(colFormHour,colGroup,colAcid))
	...
	shuffle = sorted(shuffle,reverse=True, key=operator.itemgetter(colSetAuto))#sort by priority
...
def mySlowFunction(useManOrder):
	try:
		initializedTrue
	except:
		FST.autoSchedule.initializeTest()
	...
	for x in range(lineMin1,lineMax1+1) + range(lineMin2,lineMax2+1):
		...
		try:
			...
			pass
	#sort Tables
	tablesSorted = sorted(tablesSorted,reverse=False,key=operator.itemgetter(1,10,0)) #sort by endDate, status, table
	tablesSortedLen = len(tablesSorted)
	#assign Table numbers to priority Table on screen
	#description = "Assign tables to schedule and check for exclusions"
	#steps.append(description)
	dataTag = system.tag.readBlocking(["[default]Formation/Tables/Schedule/currentScheduleFromExcelTemp","[default]Formation/Config/loadTime"])
	data = dataTag[0].value
	...
	for row in range(data.getRowCount()):
		...
		if setAutoCheck == 1:
			...
			x = 0
			table = tablesSorted[x][0]
			for j in tablesSorted: j[4] = 1 #reset exclusion column	
			...
			for i in range(len(excluGroup)): #check to see if excluGroup is in currentGroup
				if excluGroup[i] in currentGroup and excluGroup[i]:
					for j in tablesSorted:  #set exclusion bit to 0 if it finds an exclusion
			...
			#else:
				#schedLocation = "1"
			for j in tablesSorted: 	#set testFormHour column
				j[5] = formHrInt + j[2]  
			for j in tablesSorted: 	#set count column
				count = 0
				countLine1 = 0 #East = North
				countLine2 = 0 #West = South
				testItem = j[5]
				if j[2] > 0:#ignore count if timeLeft = 0 or within next hour
					for i in tablesSorted:
						if i[2] == testItem: 
...
#					else:
					...
					if forceOrder == 1:
						if table >= lineMin2:  
							lastStartimeLine2 = system.date.addMinutes(lastStartTimeLine2,int(addTime))
						else: 
							lastStartimeLine1 = system.date.addMinutes(lastStartTimeLine1,int(addTime))
							print "resetting time " + str(lastStartimeLine1)
					else:
								...
								for i in tablesSorted:
									if i[0] >=lineMin2 and i[0] <=lineMax2: #compare with line min/max
										...
										if testMinBetween < addTime:
											...
											break
								if conflict == 1:
									testStartTime = system.date.addMinutes(testStartTime,20) #add 20 minutes and redo search
								else:
					...
					if forceOrder == 0: j[9] = lastStartTime
					else: 
						if table >= lineMin2: 
					...
					tablesSorted = sorted(tablesSorted,reverse=False,key=operator.itemgetter(1,10,0))		#re-sort tableSorted Dataset
					break
			if table == 0: #if table is still zero, then go and sort tables by count,enddate,table
				#print "cmon now"
				tablesSorted = sorted(tablesSorted,reverse=False,key=operator.itemgetter(5,6,1,10,0))	
				for j in tablesSorted:
					include = j[4]
					if include == 1 and ( (checkForArea and ((line2GoLine2Only and j[0] >= lineMin2) or (line1GoLine1Only and j[0] <= lineMax1))) or not checkForArea):
						...
						if forceOrder == 1: 
							if table >= lineMin2:  
								lastStartimeLine2 = system.date.addMinutes(lastStartTimeLine2,int(addTime))
								lastStartime = lastStartimeLine2 #system.date.addMinutes(lastStartTimeLine2,int(addTime))
	
							else: 
								lastStartimeLine1 = system.date.addMinutes(lastStartTimeLine1,int(addTime))
								lastStartime = lastStartimeLine1 #system.date.addMinutes(lastStartTimeLine1,int(addTime))
						else:						
							if table >= lineMin2: 			#set lastStartTime variable for the side for this row
								...
								while conflict == 1:
									conflict = 0
									for i in tablesSorted:
										if i[0] >= lineMin2 and i[0] <=lineMax2:
											testMinBetween = abs(system.date.minutesBetween(i[9],testStartTime)) #check timeBetween
											if testMinBetween < addTime:
												#print "Found a conflicting line2 startTime, rechecking startTimes"
												conflict = 1
												break
									if conflict == 1:
										testStartTime = system.date.addMinutes(testStartTime,20) #add 20 minutes and redo search
								...
								while conflict == 1:
									conflict = 0
									for i in tablesSorted:
										if i[0] >= lineMin1 and i[0] <=lineMax1:
											testMinBetween = abs(system.date.minutesBetween(i[9],testStartTime)) #check timeBetween
											if testMinBetween < addTime:
												#print "Found a conflicting line1 startTime, rechecking startTimes"
												conflict = 1
												break
									if conflict == 1:
										testStartTime = system.date.addMinutes(testStartTime,20) #add 20 minutes and redo search
	...
	timeFromNowStartOffset = system.date.addMinutes(system.date.now(),60) #this is used to ignore the first XXXX minutes from the current time
...
#	print str(groupChanges2) + " Line 2 GroupChanges new"  
	...
	for row in range(dataCurrent.getRowCount()):  
		...
		if endTime > startTimeComparison and endTime < endTimeComparison: 		#only consider if endtime is in between start and end times
			if line == 1: tablecount1c = tablecount1c + 1  #calc tablecounts by side
			if line == 2: tablecount2c = tablecount2c + 1
				
#	print str(tablecount1c) + " Line 1 events count current"
#	print str(tablecount2c) + " Line 2 events count current"
	tablesperhour1c = tablecount1c / hoursFromNow
	tablesperhour2c = tablecount2c / hoursFromNow
...
#	print str(tablesperhour2c) + " Line 2 Tables per hour current" 
	...
	if tablesPerHourSum >= tablesPerHourSumSaved:
