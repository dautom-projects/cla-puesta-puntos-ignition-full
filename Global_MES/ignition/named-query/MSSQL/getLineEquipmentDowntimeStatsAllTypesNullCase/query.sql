SELECT 
    1 AS downtime_event,
    rt.description,
    TIMESTAMPDIFF(SECOND, :startDate, IF(:endDate > NOW(), NOW(), :endDate)) AS total_seconds,
    TIMESTAMPDIFF(SECOND, :startDate, IF(:endDate > NOW(), NOW(), :endDate)) AS down_secs,
    1 AS availability
...
AND dd.equipment_id = :equipmentId
